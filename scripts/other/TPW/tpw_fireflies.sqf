...
// VARIABLES
tpw_firefly_version = "1.03"; // Version string
	...
	[_body, _model] spawn
		...
		while {alive _model} do
			...
			if (random 1 > 0.9) then
				{
				private _buzz = format ["scripts\other\TPW\TPW_SOUNDS\sounds\fly%1.ogg",ceil (random 8)];
...
tpw_firefly_fnc_playermozzies =
	...
	if (random 1 > 0.9) then
		{
		private _buzz = format ["scripts\other\TPW\TPW_SOUNDS\sounds\mozzie%1.ogg",ceil (random 4)];
